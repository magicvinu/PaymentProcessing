...
class PaymentProcessControllerTest {
    ...
    void testClientNotFound() throws Exception {
        this.mockMvc.perform(post("/processPayments/10" ))
                .andDo(print()).andExpect(status().isNotFound())
                .andExpect(content().string(containsString("Client not found, Client with Id 10 not found")));
    }

    @Test
    void testExistingClientWhoIsAViolator() throws Exception {
        this.mockMvc.perform(post("/processPayments/9" ))
                .andDo(print()).andExpect(status().isUnprocessableEntity())
                .andExpect(content().string(containsString("Client with Id 9 have multiple payment record and marked as violator")));
    }
...
//        INSERT INTO PAYMENT_METHOD(PAYMENT_METHOD_ID, ENCRYPTED_CREDIT_CARD, ADDRESS, STATUS) VALUES(9, '4356000000000009', 'Jaddu address' , 'ACTIVE');
        ...
        int clientId = 6;
        Integer pendingOrdersBeforePaymentProcess = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
        assert pendingOrdersBeforePaymentProcess==0;
        this.mockMvc.perform(post("/processPayments/"+clientId ))
                .andDo(print()).andExpect(status().isAccepted())
                .andExpect(content().string(containsString(PaymentProcessStatus.VIOLATOR.name())));
        Boolean violator = jdbcTemplate.queryForObject("SELECT VIOLATOR FROM CLIENT WHERE CLIENT_ID=?",  new Object[] { clientId }, Boolean.class);
        assert violator == true;
        // There shouldn't be any new pending order for the violator.
        Integer newPendingOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
...
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(1, 'SUCCESS', CURRENT_TIMESTAMP);
        ...
        int clientId = 1;
        // We need to check if successfullyPaidOrderId is not added again as an unpaid order in PAYMENT_RECORD AFTER PROCESSING
        Integer successfullyPaidOrderId = jdbcTemplate.queryForObject("SELECT ORDER_ID FROM PENDING_ORDERS WHERE ORDERS_CLIENT_ID=? AND PAYMENT_RECORD_STATUS='SUCCESS'"
        ...
        Integer newPendingOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                ...
                new Object[] { clientId , successfullyPaidOrderId}, Integer.class);
...
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(5, 'DECLINE', CURRENT_TIMESTAMP);
        ...
        int clientId = 3;
        // We need to check if declinedOrderId is not added again as an unpaid order in PAYMENT_RECORD AFTER PROCESSING
        Integer declinedOrderId = jdbcTemplate
                .queryForObject("SELECT ORDER_ID FROM PENDING_ORDERS WHERE ORDERS_CLIENT_ID=? AND PAYMENT_RECORD_STATUS='DECLINE'"
                ,  new Object[] { clientId }, Integer.class);
        Integer pendingOrdersBeforePaymentProcess = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
        ...
        Integer newPendingOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                        "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                        "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=? AND PAYMENT_RECORD.ORDER_ID=?",
                new Object[] { clientId , declinedOrderId}, Integer.class);
        assert newPendingOrders==0;
...
//        -- Alan Failed payment record For current Active payment method which was processed more than 24 hr back.
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(6, 'FAIL', CURRENT_TIMESTAMP - INTERVAL 25 HOUR);
        ...
        // We need to check if failedOrderIdMoreThan24HourOld is added again as an unpaid order in PAYMENT_RECORD AFTER PROCESSING
        Integer failedOrderIdMoreThan24HourOld = jdbcTemplate
                .queryForObject("SELECT ORDER_ID FROM PENDING_ORDERS WHERE ORDERS_CLIENT_ID=? AND PAYMENT_RECORD_STATUS='FAIL'"
                        ,  new Object[] { clientId }, Integer.class);
        Integer pendingOrdersBeforePaymentProcess = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
        ...
        Integer newPendingOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                        "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                        "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=? AND PAYMENT_RECORD.ORDER_ID=?",
                new Object[] { clientId , failedOrderIdMoreThan24HourOld}, Integer.class);
        assert newPendingOrders==0;

    }

    @Test
    void testProcessOrder_FailedOrderLessThan24HourOldAreNotConsideredAsUnpaidOrders() throws Exception {
...
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(7, 'FAIL', CURRENT_TIMESTAMP);
        ...
        int clientId = 5;
        // We need to check if failedOrderIdLessThan24HourOld is not added again as an unpaid order in PAYMENT_RECORD AFTER PROCESSING
        Integer failedOrderIdLessThan24HourOld = jdbcTemplate
                .queryForObject("SELECT ORDER_ID FROM PENDING_ORDERS WHERE ORDERS_CLIENT_ID=? AND PAYMENT_RECORD_STATUS='FAIL'"
                        ,  new Object[] { clientId }, Integer.class);
        Integer pendingOrdersBeforePaymentProcess = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
        ...
        Integer newPendingOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                        "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                        "WHERE PAYMENT_RECORD.STATUS='ACTIVE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NULL AND CLIENT_ID=? AND PAYMENT_RECORD.ORDER_ID=?",
                new Object[] { clientId , failedOrderIdLessThan24HourOld}, Integer.class);
        assert newPendingOrders==0;
...
//        INSERT INTO ORDERS(ORDER_ID, CLIENT_ID, PAYMENT_METHOD_ID, TOTAL_AMOUNT, ENCRYPTED_AUTHCODE) VALUES(4, 2, 4, 20.18, 'authcode4');
        ...
        Integer successfulOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='SUCCESS' AND PAYMENT_RECORD.PROCESS_DATETIME IS NOT NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
...
//        -- Jack Failed payment record For current Active payment method which was processed more than 24 hr back.
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(10, 'FAIL', CURRENT_TIMESTAMP - INTERVAL 25 HOUR);
        ...
        Integer successfulOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='DECLINE' AND PAYMENT_RECORD.PROCESS_DATETIME IS NOT NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
        assert successfulOrders==1;
...
//        -- Rose Failed payment record For current Active payment method which was processed more than 24 hr back.
...
//        INSERT INTO PAYMENT_RECORD(ORDER_ID, STATUS, PROCESS_DATETIME) VALUES(11, 'FAIL', CURRENT_TIMESTAMP - INTERVAL 25 HOUR);
        ...
        Integer successfulOrders = jdbcTemplate.queryForObject("SELECT COUNT(1) FROM PAYMENT_RECORD \n" +
                "INNER JOIN ORDERS ON ORDERS.ORDER_ID= PAYMENT_RECORD.ORDER_ID\n" +
                "WHERE PAYMENT_RECORD.STATUS='FAIL' AND PAYMENT_RECORD.PROCESS_DATETIME IS NOT NULL AND CLIENT_ID=?",  new Object[] { clientId }, Integer.class);
