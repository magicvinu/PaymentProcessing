...
//        menu ===> [pork, beef, chicken, french fries, rice, season fruit, pizza, prawns, Water, salmon]
...
//        lamdasExamples.groupDishesWithDishTypeWithMoreThan500CaloriesUsingJava9();
    ...
    private void shortMenu() {
        System.out.println("shortMenu option 1 ==> " + menu.stream().map(Dish::getName).collect(Collectors.joining(", ")));
        System.out.println("shortMenu option 2 ==> " + menu.stream().map(Dish::getName).collect(Collectors.reducing((s1, s2) -> s1 + ", " + s2)).get());
//        shortMenu option 1 ==> pork, beef, chicken, french fries, rice, season fruit, pizza, prawns, Water, salmon
//        shortMenu option 2 ==> pork, beef, chicken, french fries, rice, season fruit, pizza, prawns, Water, salmon
    }

    private void groupByDishTypeWithName() {
        Map<Dish.Type, List<String>> groupByDishTypeWithName = menu.stream().collect(Collectors.groupingBy(Dish::getType, Collectors.mapping(Dish::getName, toList())));
...
//        groupByDishType =====> {OTHER=[french fries, rice, season fruit, pizza, Water], MEAT=[pork, beef, chicken], FISH=[prawns, salmon]}
    }

    private void noOfDishesUsingMapAndReduce() {

        int noOfDishesUsingMapAndReduce = menu.stream().mapToInt(l -> 1).reduce(Integer::sum).getAsInt();
        System.out.println("noOfDishesUsingMapAndReduce ===> " + noOfDishesUsingMapAndReduce);
    }

    private void groupByCaloriesType() {

        Map<CaloriesType, List<Dish>> groupByCaloriesType = menu.stream().collect(groupingBy(getDishCaloriesType()));
    ...
    private void averageOrSummarizingCalories() {
        System.out.println("averageCalories ==> " + menu.stream().collect(Collectors.averagingInt(Dish::getCalories)));
//        averageCalories ==> 420.0
        System.out.println("summarizingCalories ==> " + menu.stream().collect(Collectors.summarizingInt(Dish::getCalories)));
//        summarizingCalories ==> IntSummaryStatistics{count=10, sum=4200, min=0, average=420.000000, max=800}
        System.out.println("summarizingInt ==> " + menu.stream().collect(Collectors.summingInt(Dish::getCalories)));
//        summarizingInt ==> 4200
        System.out.println("summarizingInt ==> " + menu.stream().collect(Collectors.reducing(0, Dish::getCalories, (i, j) -> i + j)));
//        summarizingInt ==> 4200
    }
...
//        collect and counting => 10
        System.out.println("group by Dish type and counting => " + menu.stream().collect(groupingBy(Dish::getType, Collectors.counting())));
                ...
                menu.stream().collect(
                        partitioningBy(Dish::isVegetarian,
                                collectingAndThen(maxBy(comparingInt(Dish::getCalories)),
                                        Optional::get)));
        System.out.println("findDishWithHighestCaloriesPartitionByVegetarian ==> " + mostCaloricPartitionedByVegetarian);
//        findDishWithHighestCaloriesPartitionByVegetarian ==> {false=pork, true=pizza}
    ...
    private void findDishWithHighestCaloriesForEachDishType() {
        ...
        Map<Dish.Type, Dish> highestCaloriesForEachDishType = menu.stream()
                .collect(Collectors.groupingBy(Dish::getType, Collectors.collectingAndThen(Collectors.maxBy(comparingInt(Dish::getCalories)), Optional::get)));
        System.out.println("highestCaloriesForEachDishType ==> " + highestCaloriesForEachDishType);
//        mostCaloricByType ==> {FISH=Optional[salmon], OTHER=Optional[pizza], MEAT=Optional[pork]}
//        highestCaloriesForEachDishType ==> {FISH=salmon, OTHER=pizza, MEAT=pork}
    ...
    private void findDishWithHighestCalories() {
        ...
        Optional<Dish> highCalorieDish = menu.stream().collect(Collectors.maxBy(Comparator.comparing(Dish::getCalories)));
        System.out.println("highCalorieDish ===> " + highCalorieDish.get());
//        highCalorieDish ===> pork
        Optional<Dish> highCalorieDishUsingReduce = menu.stream().collect(Collectors.reducing((d1, d2) -> d1.getCalories() > d2.getCalories() ? d1 : d2));
...
//        highCalorieDishUsingReduce ===> pork
    ...
    private void groupByVegetarian() {
        Map<Boolean, List<Dish>> groupByVegetarian = menu.stream().collect(groupingBy(Dish::isVegetarian));
        System.out.println("groupByVegetarian ==> " + groupByVegetarian);
//        groupByVegetarian ==> {false=[pork, beef, chicken, prawns, salmon], true=[french fries, rice, season fruit, pizza, Water]}
        Map<Boolean, List<Dish>> partitionedMenuByVegetarian = menu.stream().collect(Collectors.partitioningBy(Dish::isVegetarian));
        System.out.println("partitionedMenuByVegetarian ==> " + partitionedMenuByVegetarian);
//        partitionedMenuByVegetarian ==> {false=[pork, beef, chicken, prawns, salmon], true=[french fries, rice, season fruit, pizza, Water]}

        Map<Boolean, List<Dish>> groupByVegetarianToUnmodifiableList = menu.stream().collect(groupingBy(Dish::isVegetarian, toUnmodifiableList()));
        System.out.println("groupByVegetarianToUnmodifiableList ==> " + groupByVegetarianToUnmodifiableList);
        // groupByVegetarianToUnmodifiableList ==> {false=[pork, beef, chicken, prawns, salmon], true=[french fries, rice, season fruit, pizza, Water]}
    }

    private void sumInDifferentWays() {
        System.out.println("summarizingInt ==> " + menu.stream().collect(Collectors.summarizingInt(Dish::getCalories)));
//        summarizingInt ==> IntSummaryStatistics{count=10, sum=4200, min=0, average=420.000000, max=800
        System.out.println("reducing Integer sum ==> " + menu.stream().collect(Collectors.reducing(0, Dish::getCalories, Integer::sum)));
//        reducing Integer sum ==> 4200
        System.out.println("reducing add ==> " + menu.stream().collect(Collectors.reducing(0, Dish::getCalories, (x, y) -> x + y)));
//        reducing add ==> 4200
        System.out.println("mapToInt sum ==> " + menu.stream().mapToInt(Dish::getCalories).sum());
//        mapToInt sum ==> 4200
        System.out.println("map and reduce ==> " + menu.stream().map(Dish::getCalories).reduce(Integer::sum).get());
//        map and reduce ==> 4200
    }

    private void findNumberOfDishesGroupByDishType() {
        System.out.println("findNumberOfDishesGroupByDishType option 1 ==> " +
...
//        findNumberOfDishesGroupByDishType option 1 ==> {FISH=2, OTHER=5, MEAT=3}
        System.out.println("findNumberOfDishesGroupByDishType option 2 ==> " +
...
//        findNumberOfDishesGroupByDishType option 2 ==> {FISH=2, OTHER=5, MEAT=3}
    ...
    private void groupIsVegetarianAndByDishType() {
        System.out.println("groupIsVegetarianAndByDishType using group by==> " + menu.stream().collect(Collectors.groupingBy(Dish::isVegetarian, Collectors.groupingBy(Dish::getType))));
//        groupIsVegetarianAndByDishType using group by==> {false={FISH=[prawns, salmon], MEAT=[pork, beef, chicken]}, true={OTHER=[french fries, rice, season fruit, pizza, Water]}}
        System.out.println("Or groupIsVegetarianAndByDishType using group by==> " + menu.stream().collect(Collectors.groupingBy(Dish::getType, Collectors.collectingAndThen(groupingBy(Dish::isVegetarian), t -> t))));
//        Or groupIsVegetarianAndByDishType using group by==> {FISH={false=[prawns, salmon]}, MEAT={false=[pork, beef, chicken]}, OTHER={true=[french fries, rice, season fruit, pizza, Water]}}
        System.out.println("groupIsVegetarianAndByDishType using partition ==> " + menu.stream().collect(Collectors.partitioningBy(Dish::isVegetarian, Collectors.groupingBy(Dish::getType))));
//        groupIsVegetarianAndByDishType using partition ==> {false={FISH=[prawns, salmon], MEAT=[pork, beef, chicken]}, true={OTHER=[french fries, rice, season fruit, pizza, Water]}}

    }

    private void groupByDishTypeAndIsVegetarian() {

        System.out.println("groupByDishTypeAndIsVegetarian ==> " + menu.stream().collect(groupingBy(Dish::getType, Collectors.collectingAndThen(groupingBy(Dish::isVegetarian), t -> t))));
//        groupByDishTypeAndIsVegetarian ==> {FISH={false=[prawns, salmon]}, MEAT={false=[pork, beef, chicken]}, OTHER={true=[french fries, rice, season fruit, pizza, Water]}}
        System.out.println("OR groupByDishTypeAndIsVegetarian ==> " + menu.stream().collect(groupingBy(Dish::getType, groupingBy(Dish::isVegetarian))));
//        OR groupByDishTypeAndIsVegetarian ==> {FISH={false=[prawns, salmon]}, MEAT={false=[pork, beef, chicken]}, OTHER={true=[french fries, rice, season fruit, pizza, Water]}}

    }

    private void calorieLevelByType() {

        System.out.println("calorieLevelByType option 1 ===> " + menu.stream().collect(Collectors.groupingBy(Dish::getType, Collectors.mapping(getDishCaloriesType(), Collectors.toSet()))));
        System.out.println("calorieLevelByType option 2 ===> " + menu.stream().collect(Collectors.groupingBy(Dish::getType, Collectors.mapping(getDishCaloriesType(), Collectors.toCollection(HashSet::new)))));
//        calorieLevelByType option 1 ===> {MEAT=[HIGH, LOW], OTHER=[HIGH, LOW], FISH=[MEDIUM, LOW]}
//        calorieLevelByType option 2 ===> {MEAT=[HIGH, LOW], OTHER=[HIGH, LOW], FISH=[MEDIUM, LOW]}
    ...
    private void fibonacciSequence() {
        ...
        // 1, 1, 2, 3, 5, 8, 13
        Stream.iterate(new int[]{0,1}, t -> new int[]{t[1], t[0]+t[1]}).limit(10).forEach(x -> System.out.print(x[1] + ","));
    ...
    public boolean isPrimeUsingLamdaWithOptimization(List<Integer> listOfPrimeNumbers, int candidate) {

        if(candidate <=1) return false;
        int candidateRoot = (int) Math.sqrt((double) candidate);

        return listOfPrimeNumbers.stream().filter(i -> i<=candidateRoot)
                .noneMatch(i -> candidate % i == 0);
        // java 9
...
//                .noneMatch(i -> candidate % i == 0);
    ...
    private void countByVegetarian() {
        ...
        System.out.println("countByVegetarian using group by==> " + menu.stream().collect(Collectors.groupingBy(Dish::isVegetarian, Collectors.counting())));
//        countByVegetarian using group by==> {false=5, true=5}
        System.out.println("countByVegetarian using partition ==> " + menu.stream().collect(Collectors.partitioningBy(Dish::isVegetarian, Collectors.counting())));
//        countByVegetarian using partition ==> {false=5, true=5}
    }
    private void groupByDishTypeAndVegetarianAndCalorieType() {

        System.out.println("groupByDishTypeAndVegetarianAndCalorieType ===> " + menu.stream().collect(groupingBy(Dish::getType,
                groupingBy(Dish::isVegetarian, groupingBy(getDishCaloriesType())))));
//        groupByDishTypeAndVegetarianAndCalorieType ===> {FISH={false={MEDIUM=[salmon], LOW=[prawns]}}, OTHER={true={LOW=[rice, season fruit, Water], HIGH=[french fries, pizza]}}, MEAT={false={LOW=[chicken], HIGH=[pork, beef]}}}
    ...
    private void collectionFactoryExamples(){
        ...
        try {
            List<String> friendsUsingListOf = List.of("Raphael", "Olivia", "Thibaut"); // produces Immutable list
