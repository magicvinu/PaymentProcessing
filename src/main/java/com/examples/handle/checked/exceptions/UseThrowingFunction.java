...
public class UseThrowingFunction {
  ...
  private static void
    ...
    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    List<String> dateList = asList("2020-10-11", "2020-nov-12", "2020-12-01");
    ThrowingFunction<String, Optional<Date>, ParseException> throwingFunction =
        source -> Optional.ofNullable(format.parse(source));
    Function<String, Optional<Date>> defaultFunction = (s) -> Optional.empty();
    Function<String, Optional<Date>> wrappedThrowingFunctionWithoutRethrow =
        Unchecked.apply(throwingFunction, defaultFunction);
    List<Optional<Date>> dates =
        dateList.stream().map(wrappedThrowingFunctionWithoutRethrow).collect(toList());
    System.out.println(
        "usingThrowingFunctionalInterfaceToHandleCheckedExceptionWithLamdaWithoutThrowingExceptions"
            + dates);
  }
