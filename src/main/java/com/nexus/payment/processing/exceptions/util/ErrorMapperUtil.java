...
public class ErrorMapperUtil {
    ...
    @Getter @Setter
    private static List<ErrorMetaData> errorMetadataList;
    ...
    private static ErrorMetaData genericError() {
        return ErrorMetaData.builder().category(DOWNSTREAM_ERROR_CATEGORY)
                .message("Downstream error")
                .severity(ErrorMetaData.Severity.MEDIUM)
                .code(ErrorCode.E0001)
                .source(ErrorMetaData.Source.SOURCE1).build();
    }

    public static List<Notification> remapToNotifications(Set<ErrorMetaData> errorMetaData) {

        List<ErrorMetaData> thrownErrorMetadataList = errorMetaData.stream()
    ...
    private static Function<ErrorMetaData, Notification> getNotification() {
        return emd -> Notification.builder().category(emd.getCategory())
                .code(emd.getCode().name()).message(emd.getMessage()).severity(emd.getSeverity().name())
                .source(emd.getSource().name())
                .type(emd.getType().name()).build();
    }

    public static HttpStatus getHttpStatus(Set<ErrorMetaData> errorMetaData) {
        return errorMetaData.stream().map(emd -> HttpStatus.valueOf(emd.getCategory())).findAny().orElse(HttpStatus.INTERNAL_SERVER_ERROR);
    }
