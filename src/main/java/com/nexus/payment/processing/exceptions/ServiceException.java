...
public abstract class ServiceException extends RuntimeException {
  ...
  protected ServiceException(String message, Throwable cause, ErrorMetaData errorMetaData) {
    this(
        message,
        cause,
        Optional.ofNullable(errorMetaData).map(emd -> Set.of(emd)).orElse(Collections.emptySet()));
    //        super(message, cause);
  ...
  public String getMessage() {
    return String.format(
        "%s Exception id:%s, faults:%s", super.getMessage(), exceptionId, errorMetaData.toArray());
  }
}
