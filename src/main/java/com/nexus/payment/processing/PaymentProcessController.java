...
public class PaymentProcessController {
  ...
  // One should avoid using field injection. It is always better to use constructor or setter
  // injection.
  ...
  /**
   * The processPayments does following task. 1. Finds if client is available, Throws error if it is
   * not found. 2. Identifies and marks client as violator if they have multiple active payment
   * method. The client won't be considered for future payment processing once marked violator. 3.
   * If client is not a violator. Find list of unpaid orders for processing. Call Third Party API to
   * process the order. Marked the payment_record(Unpaid orders) with the status from Third party
   * API.
   *
   * @param clientId
   * @return
  ...
  public ResponseEntity<PaymentProcessResponse> processPayments(@PathVariable Long clientId) {
    ...
    Pair<PaymentProcessStatus, String> paymentProcessStatusStringPair =
        paymentProcessService.processPayment(clientId);
