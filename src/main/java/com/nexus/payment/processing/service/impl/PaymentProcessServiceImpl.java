...
/**
 * The payment process service to perform following 1. Find and throw error if client not found. 2.
 * Identifies and marks client as violator if they have multiple active payment method. 3. If client
 * is not a violator process unpaid orders
 */
@Service
...
public class PaymentProcessServiceImpl implements PaymentProcessService {
  ...
  public Pair<PaymentProcessStatus, String> processPayment(Long clientId) {
    ...
    if (pendingOrdersList.size() == 0) {
      ...
      return Pair.of(
          PaymentProcessStatus.NO_RECORDS, "No pending orders for the client id= " + clientId);
    } else {
      // there are pending records which may have already been processed.
      pendingOrdersList.forEach(
          po -> {
            // check if order is already processed.
            if (!paymentProcessDao.findIfOrderIsAlreadyProcessed(
                po.getClientId(), po.getOrderId())) {
    ...
    if (unpaidOrderList.size() == 0) {
      ...
      return Pair.of(
          PaymentProcessStatus.NO_RECORDS, "No unpaid orders for the client id= " + clientId);
    } else {
      ...
      if (paymentProcessDao.checkIfClientIsAViolators(clientId)) {
        ...
        return Pair.of(
            PaymentProcessStatus.VIOLATOR, String.format("Client id %d is a violator", clientId));
      }

      // Get the newly created pending order for a given client
      List<UnpaidOrder> newlyCreatedUnpaidOrderList =
          paymentProcessDao.getPendingOrdersDetailsFor(clientId);

      Map<Long, PaymentAPIStatus> paymentAPIStatusMap =
          newlyCreatedUnpaidOrderList.stream()
              .collect(
                  Collectors.toMap(UnpaidOrder::getPaymentRecordId, thirdPartyService::paymentApi));
      // The better way is to use first element of unpaidOrderList, I left it that to show my
      // awareness of stream and lamda.
  ...
  /**
   * Get a lock on the client which is under process. This ensure only one server/thread can get
   * lock on the client record
   *
   * @param clientId
   */
    ...
    ClientNotFoundException clientNotFoundException =
        new ClientNotFoundException(
            ...
            ErrorMetaData.builder()
                ...
                .category(HttpStatus.NOT_FOUND.value())
                .build());

    AssertUtil.check(
        clientId,
        c -> paymentProcessDao.checkIfClientExistByLockingRecord(c),
