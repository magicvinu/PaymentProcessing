...
public class PaymentProcessDaoImpl extends JdbcDaoSupport implements  PaymentProcessDao{
    ...
    public boolean checkIfClientIsAViolators(Long clientId) {
        ...
        String isClientAViolatorQuery = "SELECT count(*) >1 FROM CLIENT \n" +
        ...
        boolean isClientAViolator = getJdbcTemplate().queryForObject(isClientAViolatorQuery, new Object[] { clientId }, Boolean.class);
    ...
    public boolean findIfOrderIsAlreadyProcessed(Long clientId, Long orderId){
        String pendingOrderQuery="SELECT count(*) >=1 FROM PENDING_ORDERS WHERE ORDER_ID=? AND ORDERS_CLIENT_ID=?" +
                " AND PAYMENT_RECORD_STATUS='SUCCESS' AND PAYMENT_RECORD_PROCESS_DATETIME IS NOT NULL";
        boolean orderAlreadyProcessed = getJdbcTemplate().queryForObject(pendingOrderQuery, new Object[] { orderId, clientId }, Boolean.class);
        return orderAlreadyProcessed;
    }

    public List<PendingOrder> findPendingOrdersFor(Long clientId){
        String pendingOrderQuery="SELECT * FROM PENDING_ORDERS WHERE ((PAYMENT_RECORD_STATUS IS NULL AND PAYMENT_METHOD_STATUS= 'ACTIVE')\n" +
        ...
        return getJdbcTemplate().query(pendingOrderQuery, new Object[] { clientId }, new PendingOrderRowMapper());
    }
    ...
    public void insertPendingOrdersFor(Long clientId){
        ...
        String insertPendingOrderQuery = "INSERT  INTO PAYMENT_RECORD (ORDER_ID, STATUS, PROCESS_DATETIME)\n" +
    ...
    public void insertUnpaidOrders(PendingOrder pendingOrder){
        String insertUnpaidOrderQuery = "INSERT  INTO PAYMENT_RECORD (ORDER_ID, STATUS, PROCESS_DATETIME) " +
                "VALUES(?,'ACTIVE', null)";
        int insertRowCount = getJdbcTemplate().update(insertUnpaidOrderQuery, pendingOrder.getOrderId());
        log.info("Number of records inserted " + insertRowCount);
    }
    /**
    ...
    public List<UnpaidOrder> getPendingOrdersDetailsFor(Long clientId) {
        String unpaidOrderQuery="SELECT PAYMENT_RECORD.PAYMENT_RECORD_ID,TOTAL_AMOUNT,ENCRYPTED_CREDIT_CARD,ENCRYPTED_AUTHCODE,ORDERS.CLIENT_ID FROM PAYMENT_RECORD \n" +
        ...
        return getJdbcTemplate().query(unpaidOrderQuery, new Object[] { clientId }, new UnpaidOrderRowMapper());
     ...
     */
    @Override
    public void updatePaymentRecordWithStatusFromPaymentApi(Map<Long, PaymentAPIStatus> paymentAPIStatusMap) {

        String updatePaymentApiResponseStatusQuery = "UPDATE PAYMENT_RECORD SET STATUS=?, PROCESS_DATETIME=CURRENT_TIMESTAMP WHERE PAYMENT_RECORD_ID=?";
        paymentAPIStatusMap.keySet().stream().forEach(x -> {
            getJdbcTemplate().update(updatePaymentApiResponseStatusQuery, paymentAPIStatusMap.get(x.longValue()).name(), x.longValue());
        });
